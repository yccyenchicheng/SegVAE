# Ref: https://github.com/hubert0527/COCO-GAN/blob/master/img_utils.py
...
def get_celebahqmask_image(image_path, input_height, input_width, resize_height=64, resize_width=64, byte_image=True):
    image = imread(image_path).astype(np.float32)
    return transform(image, input_height, input_width, resize_height, resize_width, byte_image=byte_image)

def get_humanparsing_image(image_path, input_height, input_width, resize_height=64, resize_width=64, byte_image=True):
    image = imread(image_path).astype(np.float32)
    return transform(image, input_height, input_width, resize_height, resize_width, byte_image=byte_image)
